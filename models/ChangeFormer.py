...
class DecoderTransformer_v3(nn.Module):
    ...
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        ############################ 3d预测头
        self.regressor = ConvLayer(in_channels=self.embedding_dim, out_channels=1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, inputs1, inputs2):
        ...
        ################################my my begin
        cp2d = self.change_probability(x)
        if self.output_softmax:
            cp2d = self.active(cp2d)

        cp3d = self.regressor(x)
        cp3d = self.active3d(cp3d)
...
class ChangeFormerV6(nn.Module):
    ...
    def __init__(self, input_nc=3, output_nc=2, img_size= 256, decoder_softmax=False, embed_dim=256):
        ...
        self.Tenc_x2 = EncoderTransformer_v3(img_size=self.img_size, patch_size = 7, in_chans=input_nc, num_classes=output_nc, embed_dims=self.embed_dims,
    ...
    def forward(self, x1, x2):
        ...
        ################################## my my begin 
        cp2d, cp3d = self.TDec_x2(fx1, fx2)
        return cp2d, cp3d
        ################################## my my end
